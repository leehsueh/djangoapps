 ...
 *
 * Modified by Hain-Lee to customize post URL for use with django (1/17/2011)
 *
 */
		...
		return this.each(function(options) {
			...
			$("#contactForm").validate({
				...
				submitHandler: function() {
					...
					$('#loading').show();
                                        var successCallback = function(data){
                                                ...
                                                if( data == 'success') {
                                                    ...
                                                    $('#loading').css({display:'none'});
                                                    $('#callback').show().append(defaults.notRecievedMsg + ' ' + data);
                                                }
                                        };
                                        var errorCallback = function(xmlRequest, textStatus, errorThrown) {
                                            $('#callback').show().append(defaults.notRecievedMsg + ': ' + textStatus + ' ' + errorThrown);
                                        }
                                        $.ajax({
                                            type: 'get',
                                            url: defaults.actionUrl,
					...
					//$.get(defaults.actionUrl,{subject:defaults.subject, name:$('#name').val(), email:$('#email').val(), comment:$('#comment').val()}, successCallback);
				}
			});
		});
	};
